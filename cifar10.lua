...
local root_dir='/media/sdj/Open_Datasets/'
if not paths.dirp(root_dir .. 'cifar-10-batches-t7') then
   os.execute('wget ' .. tar)
...
for i = 0,4 do
   subset = torch.load(root_dir .. 'cifar-10-batches-t7/data_batch_' .. (i+1) .. '.t7', 'ascii')
   trainData.data[{ {i*10000+1, (i+1)*10000} }] = subset.data:t()
   trainData.labels[{ {i*10000+1, (i+1)*10000} }] = subset.labels
end
trainData.labels = trainData.labels + 1

subset = torch.load(root_dir .. 'cifar-10-batches-t7/test_batch.t7', 'ascii')
testData = {
